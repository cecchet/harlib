...
public class HarCacheRequest
{
   ...
   * @throws IOException
   */
  public HarCacheRequest(JsonParser jp, boolean isBefore, List<HarWarning> warnings)
      throws JsonParseException, IOException
  {
    ...
    if (jp.nextToken() != JsonToken.START_OBJECT)
    {
      if ("null".equals(jp.getText()))
        return;
      throw new JsonParseException("{ missing after \"" + getObjectTag() + "\" element", jp.getCurrentLocation());
    }
    ...
    while (jp.nextToken() != JsonToken.END_OBJECT)
    {
      ...
      else if ("lastAccess".equals(name))
      {
        ...
        catch (ParseException e)
        {
          ...
          try
          {
            setLastAccess(SDF.parse(jp.getText()));
          }
          catch (ParseException e1)
          {
            if (warnings != null)
      ...
      else if ("lastFetched".equals(name))
      {
        try
        {
          if (jp.getCurrentName().equals(jp.getText()))
            jp.nextToken();
          setLastFetch(ISO8601DateFormatter.parseDate(jp.getText()));
        }
        catch (ParseException e)
        {
          // Fallback try 1.2 spec without timezone
          try
          {
            setLastFetch(SDF.parse(jp.getText()));
          }
          catch (ParseException e1)
          {
            if (warnings != null)
      ...
      else if ("fetchCount".equals(name))
        setFetchCount(jp.getValueAsInt());
      else if (name != null && name.startsWith("_"))
        this.customFields.addHarCustomFields(name, jp);
      else
      {
        throw new JsonParseException("Unrecognized field '" + name + "' in " + getObjectTag() + " element",
  ...
  public HarCacheRequest(HarDatabaseConfig config, long cacheId, boolean isBefore) throws SQLException
  {
    ...
    try
    {
      ps = c.prepareStatement(
          "SELECT expires,last_access,etag,hit_count,comment FROM " + tableName + " WHERE cache_id=? AND is_before=?");
      ps.setLong(1, cacheId);
      ps.setInt(2, isBefore ? 1 : 0);
      rs = ps.executeQuery();
      if (!rs.next())
        throw new SQLException(
            "No " + getObjectTag() + " HarCacheRequest for cache id " + cacheId + " found in database");
   ...
   * @throws SQLException if a database error occurs
   */
  public void writeJDBC(long cacheId, HarDatabaseConfig config, PreparedStatement cachePs, long logId)
      throws SQLException
  {
    if (eTag == null)
    { // If mandatory field is null, it was a null object
  ...
  /**
   * Returns the lastFetch value.
   * 
   * @return Returns the lastFetch.
   */
  ...
  {
    return "\"" + getObjectTag() + "\":{ \"expires\": \"" + ISO8601DateFormatter.format(expires)
        + "\",\"lastAccess\": \"" + ISO8601DateFormatter.format(lastAccess) + "\",\"eTag\": \"" + eTag
